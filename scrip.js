...
function checkLaserCollision(laser, alien) {
    ...
    if(laserLeft != 340 && laserLeft + 40 >= alienLeft) {
        if(laserTop <= alienTop && laserTop >= laserBottom) {
            return true;
        } else {
            return false;
        }
...
function playGame() {
    startButtom.style.display = 'nome';
    instructionText.style.display = 'nome';
    window.addEventListener('keydown', flyShip);
    moveAlienInterval = setInterval(() => {
        createAlien();
    }, 2000)
}
